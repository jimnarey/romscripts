...
class Rom:
    ...
    def is_commercial(self):
        if True in (self.is_hacked_dump, self.is_hack_game, self.is_beta, self.is_unlicensed, self.is_pd):
            return False
        return True
...
class MultiRomArchive:
    ...
    def get_best_from_region(self, region):
        region_matches = [rom for rom in self.roms if rom.region == region and rom.is_commercial()]
        ...
        for root, dirs, files in os.walk(self.path):
            for file in files:
                if py7zr.is_7zfile(os.path.join(root, file)):
                    self.archives.append(MultiRomArchive(os.path.join(root, file)))
...
def main(input_dir, output_dir):
    ...
    for archive in archive_dir.archives:
        for rom in archive.get_trainers():
            archive.extract_entry(os.path.join(output_dir, 'Trainers'), rom.rom_name)
        for rom in archive.get_hacks():
            archive.extract_entry(os.path.join(output_dir, 'Hacks'), rom.rom_name)
        for rom in archive.get_translations():
            archive.extract_entry(os.path.join(output_dir, 'Translations'), rom.rom_name)
        for region in archive_dir.regions:
            best_region_match = archive.get_best_from_region(region)
            if best_region_match:
                archive.extract_entry(os.path.join(output_dir, region), best_region_match.rom_name)
